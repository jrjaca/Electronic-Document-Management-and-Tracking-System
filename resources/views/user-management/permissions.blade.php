...
@section('content')
    ...
    @component('common-components.breadcrumb')
        @slot('title') PERMISSIONS (FOR DEVELOPER ONLY) @endslot
        @slot('li_1') <a title="Back to Home" href="{{route('index')}}">Home</a> @endslot
        @slot('li_2') List of Permissions @endslot
    @endcomponent  
    ...
    <!-- END Display Message-->
                        ...
                        @can('permission_store')
                            <div style="float: left;">
                                <button type="button" class="btn btn-outline-primary btn-sm waves-effect waves-light"
                                    data-toggle="modal" data-target=".addModal"> Add New (Please notify the developer)
                                </button>
                            </div><br/><br/><br/>
                        @endcan   
                        ...
                        <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            <thead>
                                <tr>
                                    ...
                                    <th>Slug Name</th>
                                    <th>Status</th>
                                    @if(Gate::check('permission_view') || Gate::check('permission_disable') || Gate::check('permission_enable'))
                                        <th>Action</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                @if(Gate::check('permission_view') || Gate::check('permission_disable') || Gate::check('permission_enable'))
                                    ...
                                    @if( $row->deleted_at == null )
                                        @can('permission_view')
                                            <a href="javascript:void(0)" onclick="showPermissionDetails('{{Hasher::encode($row->id)}}');"> 
                                                <span style="font-size: 1.2em; color: Dodgerblue;">
                                                    <i class="fa fa-edit" title="Edit {{ $row->title }}"></i>
                                                </span>
                                            </a>&ensp;
                                        @endcan     
                                        @can('permission_disable')
                                            <a href="{{ route('manage.permissions.disable', ['id' => Hasher::encode($row->id)]) }}" id="sa-custom-enableSoftDelete">
                                                <span style="font-size: 1.2em; color: Red;">
                                                    <i class="fa fa-lock" title="Disable {{ $row->title }}"></i>
                                                </span></a>&ensp;
                                        @endcan   
                                    @else
                                        @can('permission_enable')
                                            <a href="{{ route('manage.permissions.enable', ['id' => Hasher::encode($row->id)]) }}" id="sa-custom-disableSoftDelete">
                                                <span style="font-size: 1.2em; color: Dodgerblue;">
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addModalLabel">PERMISSION</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form id="my_form" action="{{ route('manage.permissions.save') }}" method="POST" class="needs-validation" novalidate>                       
                        @csrf
        ...
        <div id="editModal" class="modal fade editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModalLabel">UPDATE PERMISSION</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form id="my_form" action="{{ route('manage.permissions.update') }}" method="POST" class="needs-validation" novalidate>                       
                        @csrf
                        <div class="modal-body">
...
@section('script')
    ...
    <script>        
        function showPermissionDetails(id){
            ...
            $.ajax({
                type:"GET",
                url: "{{ url('user-management/permissions/edit/') }}"+"/"+id,  
                dataType: "json",
                cache: false,
