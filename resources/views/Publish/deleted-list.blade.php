...
@section('title') List of Unpublished/Disabled Documents @endsection
...
@section('content')
    ...
    @component('common-components.breadcrumb')
        @slot('title') UNPUBLISHED/DISABLED DOCUMENTS @endslot
        @slot('li_1') <a title="Back to Home" href="{{route('index')}}">Home</a> @endslot
        @slot('li_2') List of Unpublished/Deleted Documents @endslot
    @endcomponent  
                        ...
                        <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            <thead>
                                <tr>
                                    ...
                                    {{-- <th>Published To</th> --}}
                                    <th>Date and Time Unpublished</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody> 
                                ...
                                <tr>
                                    ...
                                    <td>  
                                        @if(Gate::check('view_publish_document') || Gate::check('view_deleted_publish_document'))
                                        ...
                                        @if(Gate::check('publish_document'))
                                            <a href="{{ route('publish.enable', ['id' => Hasher::encode($row->published_id)]) }}" id="sa-custom-disableSoftDelete">
                                                <span style="font-size: 1.2em; color: Blue;">
                                                    <i class="fa fa-unlock" title="Publish/Enable {{ $row->title }}."></i>
                                                </span></a> &nbsp;  
                                                {{-- <i class="fas fa-upload"></i>fas fa-file-upload --}}
                                        @endif
...
<!-- sweetalert -->
        ...
        <!-- Published/enable  document -->
        <script>
            $(document).on("click", "#sa-custom-disableSoftDelete", function(e){
                e.preventDefault();
                var link = $(this).attr("href");
                swal({
                        title: "Are you sure want to re-publish?",
                        text: "",
                        icon: "warning",
                        buttons: true,
                        dangerMode: true,
