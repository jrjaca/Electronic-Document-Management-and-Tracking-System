...
@section('title') List of Document Actions @endsection
...
@section('content')
    ...
    @component('common-components.breadcrumb')
        @slot('title') DOCUMENT ACTIONS @endslot
        @slot('li_1') <a title="Back to Home" href="{{route('index')}}">Home</a> @endslot
        @slot('li_2') List of Document Actions @endslot
    @endcomponent  
    ...
    <!-- END Display Message-->
                        ...
                        <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            <thead>
                                <tr>
                                    ...
                                    <th>Description</th>
                                    <th>Status</th>
                                    @if(Gate::check('documentaction_update') || Gate::check('documentaction_disable') || Gate::check('documentaction_enable'))
                                        <th>Action</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($docactions as $key => $row)  
                                <tr 
                                ...
                                @if(Gate::check('documentaction_update') || Gate::check('documentaction_disable') || Gate::check('documentaction_enable'))
                                    ...
                                    @if( $row->deleted_at == null )
                                        @can('documentaction_update')
                                            <a href="javascript:void(0)" onclick="showDocActionDetails('{{Hasher::encode($row->id)}}');"> 
                                                <span style="font-size: 1.2em; color: Dodgerblue;">
                                                    <i class="fa fa-edit" title="Edit {{ $row->title }}"></i>
                                                </span>
                                            </a>&ensp;
                                        @endcan     
                                        @can('documentaction_disable')
                                            <a href="{{ route('manage.docactions.disable', ['id' => Hasher::encode($row->id)]) }}" id="sa-custom-enableSoftDelete">
                                                <span style="font-size: 1.2em; color: Red;">
                                                    <i class="fa fa-lock" title="Disable {{ $row->title }}"></i>
                                                </span></a>&ensp;
                                        @endcan   
                                    @else
                                        @can('documentaction_enable')
                                            <a href="{{ route('manage.docactions.enable', ['id' => Hasher::encode($row->id)]) }}" id="sa-custom-disableSoftDelete">
                                                <span style="font-size: 1.2em; color: Dodgerblue;">
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addModalLabel">DOCUMENT ACTION</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form id="my_form" action="{{ route('manage.docactions.save') }}" method="POST" class="needs-validation" novalidate>                       
                        @csrf
        ...
        <div id="editModal" class="modal fade editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModalLabel">UPDATE DOCUMENT ACTION</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form id="my_form" action="{{ route('manage.docactions.update') }}" method="POST" class="needs-validation" novalidate>                       
                        @csrf
                        <div class="modal-body">
...
@section('script')
    ...
    <!-- Edit Permission -->
    <script>        
        function showDocActionDetails(id){
            spinner('Loading...');
            $.ajax({
                type:"GET",
                url: "{{ url('document-action-management/doc-actions/edit/') }}"+"/"+id,  
                dataType: "json",
                cache: false,
